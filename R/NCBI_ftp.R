...
.downloadData <- function(file, tax_id, NCBIFilesDir, rebuildCache, verbose)
{
    ...
    ## download
    if (rebuildCache) {
        url <- paste("ftp://ftp.ncbi.nlm.nih.gov/gene/DATA/",names(file),
                     sep="")
    ...
    function(con,table, field)
{
    ...
    dbGetQuery(con,
               paste("CREATE INDEX IF NOT EXISTS", indName,
                     "ON", table, "(", field, ")"))
}
              ...
              NCBIFilesDir, rebuildCache, verbose)
{
    ...
    }else if(dim(data)[1] == 0 && names(file)=="gene2go.gz"){ ## need blast2GO
    ...
    for(i in 1:times){
        ...
        if(is(tryResult,"try-error") && i < times){
            ...
            msg = paste("After 2 attempts, AnnotationDbi is still not able",
                        "to access the following URL:", url,
                        "You might want to try again later since this means "
                        "you will not have GO data for your package.",
                        sep=" ")
