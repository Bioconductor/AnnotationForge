...
## new test to verify probes and genes exist 
getProbes <- function(){
    require("@PKGNAME@")
...
getGenes <- function(){
    require("@PKGNAME@")
    as.numeric(dbGetQuery(dbconn(@PKGNAME@),
                 "SELECT count(DISTINCT systematic_name) FROM probes"))
}
msg = paste("This package has no systematic names.",
...
getGenesOther <- function(){
    require("@PKGNAME@")
    as.numeric(dbGetQuery(dbconn(@PKGNAME@),
                 "SELECT count(DISTINCT sgd_id) FROM probes"))
}
msg = paste("This package has no SGD IDs.",
  " This can be caused by a large number ",
  "of upstream changes. But it's usually caused by alterations to the source ",
  "files used to extrac the probes at the very beginning.", sep="")
checkTrue(getGenesOther() > 0,
          msg = paste(strwrap(msg, exdent=2),collapse="\n") )
...
getGeneNames <- function(){
    require("@PKGNAME@")
    as.numeric(dbGetQuery(dbconn(@PKGNAME@),
                 "SELECT count(DISTINCT gene_name) FROM probes"))
}
msg = paste("This package has no gene names.",
  " This can be caused by a large number ",
  "of upstream changes. But it's usually caused by alterations to the source ",
  "files used to extrac the probes at the very beginning.", sep="")
checkTrue(getGeneNames() > 0,
          msg = paste(strwrap(msg, exdent=2),collapse="\n") )
