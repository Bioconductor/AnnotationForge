...
\Rmethod{select} interface.  The 1st one is \Rmethod{columns}, which
...
\Rmethod{keytype},\Rmethod{columns},\Rmethod{keys} and \Rmethod{select}
...
\subsection{Example: creating \Rmethod{keytypes} and \Rmethod{columns} methods}

The \Rmethod{keytype} and \Rmethod{columns} methods are always the 1st
...
@
%%
Also, notice how for both \Rmethod{keytypes} and \Rmethod{columns} I am
...
\subsection{Examples: creating a \Rmethod{columns} and \Rmethod{keytypes} method}

Now lets suppose that we want to define a \Rmethod{columns} method for
