...
now the \Rmethod{select} method.  The interface provides a simple way
of extracting data.  

There are really 4 methods that work together to allow a
\Rmethod{select} interface.  The 1st one is \Rmethod{cols}, which
...
The next method is \Rmethod{keytypes} which tells you the kinds of
...
The third method is \Rmethod{keys} which is used to retrieve all the
...
And finally there is \Rmethod{select}, which extracts data by using
...
% Val: This would probably be a good place to discuss XML parsing (if
...
purely for dispatch.  We just need \Rmethod{select} and it's friends
...
\Rmethod{keytype},\Rmethod{cols},\Rmethod{keys} and \Rmethod{select}
...
\subsection{Example: creating \Rmethod{keytypes} and \Rmethod{cols} methods}

The \Rmethod{keytype} and \Rmethod{cols} methods are always the 1st
ones you should implement.  They are the easiest, and their existence
is required to be able to use \Rmethod{keys} or \Rmethod{select}.  In
...
So what about cols?  Well it's not a whole lot more complicated in
...
@
%%
Also, notice how for both \Rmethod{keytypes} and \Rmethod{cols} I am
using all capital letters.  This is a style adopted throughout the
project.

\subsection{Example 2: creating a \Rmethod{select} method}
...
default \Rfunarg{keytype} of "UNIPROT".
\end{Exercise}
\begin{Solution}
<<webServiceSelect>>=
...
pair of useful methods are the \Rmethod{dbListTables} and
\Rmethod{dbListFields} which are useful for exploring the schema of a
database.
...
\end{verbatim}
So we can now call a query like that from R by using \Rmethod{dbGetQuery}: 
<<dbGetQuery2>>=
...
\subsection{Examples: creating a \Rmethod{cols} and \Rmethod{keytypes} method}
...
been done before for the other \Rmethod{select} interfaces.  But doing
...
\subsection{Example: creating a \Rmethod{keys} method}

\begin{Exercise}
  Now define a method for \Rmethod{keys} on \Rclass{hom.Hs.inp.db}.
